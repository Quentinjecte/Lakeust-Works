<!--Ordre de navigation (clavier / sommaire) : Catalogue d'abord, puis
    les quinze mécaniques. Doit rester synchronisé avec ORDER dans
    resources/js/labs/scroll/scroll-lab.js. -->

@extends('layouts.site')
@section('title', 'Scroll Lab — Lakeust Works')
@section('content')
<head>
    @vite(['resources/css/lab.css', 'resources/js/labs/scroll/scroll-lab.js'])
    ...
    @php
        ...
        $sections = [
            ['tag' => 'X', 'name' => 'Catalogue', 'type' => '', 'built' => 0,],
            ['tag' => 'Ω · 01', 'name' => 'Chambre orbitale', 'built' => 6, 'status' => 'Prototypé', 'i18n' => 'scroll.cat.1',
            ...
            ['tag' => 'Ω · 02', 'name' => 'Champ polarisé', 'built' => 7, 'status' => 'Prototypé', 'i18n' => 'scroll.cat.2',
            ...
            ['tag' => 'Ω · 03', 'name' => 'Corridor', 'built' => 8, 'status' => 'Prototypé', 'i18n' => 'scroll.cat.3',
            ...
            ['tag' => 'Ω · 04', 'name' => 'Démontage', 'built' => 9, 'status' => 'Prototypé', 'i18n' => 'scroll.cat.4',
            ...
            ['tag' => 'Ω · 05', 'name' => 'Quadrants contrariés', 'built' => 10, 'status' => 'Prototypé', 'i18n' => 'scroll.cat.5',
            ...
            ['tag' => 'Ω · 06', 'name' => 'Morphogenèse', 'built' => 11, 'status' => 'Prototypé', 'i18n' => 'scroll.cat.6',
            ...
            ['tag' => 'Ω · 07', 'name' => 'Entrelacs', 'built' => 12, 'status' => 'Prototypé', 'i18n' => 'scroll.cat.7',
            ...
            ['tag' => "Ω · 08", 'name' => "Repli", 'built' => 13, 'status' => "Prototypé", 'i18n' => 'scroll.cat.8',
            ...
            ['tag' => "Ω · 09", 'name' => "Focale verticale", 'built' => 14, 'status' => "Prototypé", 'i18n' => 'scroll.cat.9',
            ...
            ['tag' => 'Ω · 10', 'name' => 'Mémoire inverse', 'built' => null, 'status' => 'Fiche', 'i18n' => 'scroll.cat.10',
            ...
            ['tag' => 'Ω · 11', 'name' => 'Gravité réglable', 'built' => null, 'status' => 'Fiche', 'i18n' => 'scroll.cat.11',
            ...
            ['tag' => 'Ω · 12', 'name' => 'Sédiments', 'built' => null, 'status' => 'Fiche', 'i18n' => 'scroll.cat.12',
            ...
            ['tag' => 'Ω · 13', 'name' => 'Fréquence', 'built' => null, 'status' => 'Fiche', 'i18n' => 'scroll.cat.13',
            ...
            ['tag' => 'Ω · 14', 'name' => 'Scroll désynchronisé', 'built' => null, 'status' => 'Fiche', 'i18n' => 'scroll.cat.14',
            ...
            ['tag' => 'Ω · 15', 'name' => 'Plan de netteté', 'built' => null, 'status' => 'Fiche', 'i18n' => 'scroll.cat.15',
...
<div data-lab-root style="position:relative;min-height:100vh;background:#05050a">
    ...
    <nav style="position:fixed;left:clamp(10px,2.4vw,26px);top:50%;transform:translateY(-50%);z-index:50;display:flex;flex-direction:column;gap:0;padding-left:12px">
        ...
        @foreach ($sections as $r)
            <button data-rail title="{{ $r['name'] }}" class="lab-rail-btn" style="appearance:none;background:none;border:0;cursor:pointer;display:flex;align-items:center;height:26px;padding:0;color:rgba(226,221,209,.40);font-family:inherit;font-size:10px;letter-spacing:.20em;text-transform:uppercase;transition:color .3s">
                <span>{{ $r['tag'] }}</span>
            </button>
        @endforeach
    </nav>
    ...
    <main style="position:relative;z-index:1">
        ...
        <section class="lab-sec" data-lab="catalogue" data-screen-label="Lab IX Catalogue">
            <div style="min-height:52vh;display:flex;flex-direction:column;justify-content:flex-end;gap:20px;padding:0 clamp(56px,9vw,180px) clamp(40px,6vw,80px)">
                <span data-reveal="line" style="display:block;width:clamp(80px,12vw,180px);height:1px;background:linear-gradient(90deg,#9184d9,transparent)"></span>
                <h1 data-chars style="margin:0;font-size:clamp(34px,6.2vw,86px);font-weight:200;letter-spacing:-.03em;line-height:.98" data-i18n="lab.title.catalogue">Catalogue</h1>
                <p data-reveal="up" style="margin:0;max-width:56ch;font-size:clamp(14px,1.1vw,16px);line-height:1.7;color:rgba(226,221,209,.62)" data-i18n="scroll.intro">Quinze mécaniques de scroll inventées pour ce laboratoire, au-delà des six systèmes de base. Neuf sont prototypées et navigables ; six restent à l'état de fiche. Chaque fiche indique ce que le scroll pilote, ce que l'on voit, la difficulté réelle et l'intérêt pour le portfolio.</p>
            </div>
                ...
                @foreach (array_slice($sections, 1, count($sections) - 1, true) as $k => $c)
                    <article style="display:flex;flex-direction:column;gap:14px;padding:24px 24px 26px;border:1px solid rgba(233,233,237,.10);background:rgba(11,12,20,.55)">
                        <div style="display:flex;align-items:baseline;justify-content:space-between;gap:12px">
                            ...
                            <span style="font-size:10px;letter-spacing:.20em;text-transform:uppercase;color:rgba(226,221,209,.34)" data-i18n="{{ $c['status'] === 'Fiche' ? 'lab.status.sheet' : 'lab.status' }}">{{ $c['status'] }}</span>
                        </div>
                        ...
                        <div style="display:flex;flex-direction:column;gap:9px;padding-top:14px;border-top:1px solid rgba(233,233,237,.08)">
                            <span style="display:grid;grid-template-columns:80px 1fr;gap:14px;font-size:11.5px;line-height:1.55"><span style="letter-spacing:.16em;text-transform:uppercase;color:rgba(226,221,209,.30)" data-i18n="lab.field.scroll">Scroll</span><span style="color:rgba(226,221,209,.72)" data-i18n="{{ $c['i18n'] }}.scroll">{{ $c['scroll'] }}</span></span>
                            <span style="display:grid;grid-template-columns:80px 1fr;gap:14px;font-size:11.5px;line-height:1.55"><span style="letter-spacing:.16em;text-transform:uppercase;color:rgba(226,221,209,.30)" data-i18n="lab.field.seen">Vu</span><span style="color:rgba(226,221,209,.72)" data-i18n="{{ $c['i18n'] }}.seen">{{ $c['seen'] }}</span></span>
                            ...
                            <span style="display:grid;grid-template-columns:80px 1fr;gap:14px;font-size:11.5px;line-height:1.55"><span style="letter-spacing:.16em;text-transform:uppercase;color:rgba(226,221,209,.30)" data-i18n="lab.field.cost">Coût</span><span style="color:rgba(226,221,209,.72)" data-i18n="{{ $c['i18n'] }}.cost">{{ $c['cost'] }}</span></span>
                            <span style="display:grid;grid-template-columns:80px 1fr;gap:14px;font-size:11.5px;line-height:1.55"><span style="letter-spacing:.16em;text-transform:uppercase;color:rgba(226,221,209,.30)" data-i18n="lab.field.portfolio">Portfolio</span><span style="color:rgba(226,221,209,.72)" data-i18n="{{ $c['i18n'] }}.use">{{ $c['use'] }}</span></span>
                        </div>
                        @if ($c['built'] !== null)
                            <button data-cta="{{ $c['built'] }}" class="lab-cta" style="appearance:none;margin-top:6px;align-self:flex-start;background:none;border:1px solid rgba(145,132,217,.5);color:#b3a9e6;cursor:pointer;font-family:inherit;font-size:10px;letter-spacing:.22em;text-transform:uppercase;padding:9px 16px;border-radius:8px;transition:background .3s,color .3s"><span data-i18n="lab.open">Ouvrir</span> {{ str_pad($c['built'] + 1, 2, '0', STR_PAD_LEFT) }}</button>
                        @endif
                    </article>
                @endforeach
            </div>
